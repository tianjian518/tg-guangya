...
_NOISE_WORDS = re.compile(
    ...
    # 光鸭分享频道广告词（2026-09 频道实测：「无任何广-4K」「纯净」「4KMAX画质 3G/集」）
    r"无任何广告|无任何广|无广告|无广|纯净|画质|4kmax|中文|"
    r"\d{1,3}\s*[GMB]i?B?\s*/?\s*[集话話])",
    re.I,
)
...
# ⚠️ 追更写法（首更至3集 / 更10集 / 4稍后）来自光鸭分享频道实测：sig 由
#    _EP_CN_BARE 从原文提前解析（S01E03），这里只负责把词从 core 里剥掉。
# ⚠️ 光数字追更（「早春晴朗更新至17」不带「集」字）也是频道实测形态：
#    漏剥会把整串脏词喂给 TMDB 模糊搜索，命中错误条目导致地区误判（欧美剧 ← 国产剧）。
#    光数字分支必须放在带「集/话/期」分支之后——交替从左到右，
#    「更新至17集」优先整段剥掉，不能留下光数字只剥一半。
...
def _lookup_en(core: str, year: int = 0) -> tuple[str, str, int]:
    ...
    年份只在标题没写年份时用来补全「片名 (年份)」的 Emby 文件夹命名。
...
def analyze(title: str) -> ResourceInfo:
    ...
    if not re.search(r"[一-鿿]", info.core or ""):
        ...
        # 标题没带年份 → 拿 TMDB 的上映/首播年份补上（Emby 文件夹命名要用）
    ...
    if not meta_region and core_en and not translated_from_en:
        _, meta_region, tmdb_year = _lookup_en(core_en, info.year)
        # 英文原名查出的年份同样借来补全（标题没写年份时）
        if not info.year and tmdb_year:
            info.year = tmdb_year
    # 地区提示。两条来源，前者优先：
