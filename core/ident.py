...
# 前缀允许点前是字母/中文（压制组普遍写成 The.Matrix.1999.1080p / Oppenheimer.2023.BluRay，
# 点号分隔的年份必须认）；只防「数字+点」开头（1.2023 这种是版本号不是年份）。
# ⚠️ 旧写法 (?<![\d.]) 会把所有点号分隔的年份都排除掉，年份被当成片名一部分，
#    结果是「奥本海默2023」这种粘连命名，且 year=0 让同片不同写法对不齐去重键。
_YEAR = re.compile(r"(?<!\d)(?<!\d\.)(?:19|20)\d{2}(?!\d(?:\.\d+)?)")
