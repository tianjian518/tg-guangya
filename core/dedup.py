...
# 落盘准入检查。
#
# ⚠️ 需求变更（重要）：用户要求「频道链接与机器人链接都要改成中文并落到云盘」。
# 旧实现把「译不出中文片名」当成放弃理由，等于**白白丢掉资源**——
# 这比落一个英文名糟糕得多（用户拿不到片，字典也永远补不全）。
# 所以现在只剩两种真正没法落的阻断：
#   ① 自动整理被关闭 —— 没有分类目录可放，只能裸丢根目录；
#   ② 分类结果为空 / 落入「未分类」兜底桶 —— 等于没整理成功。
# 译不出中文的情况改为：允许落盘（用清理后的英文原名），由调用方写进
# 「待翻译清单」，用户补字典或配好 TMDB 后可批量改名。
def _standard_block_reason(info, cr, cat: str, organize_enabled: bool,
                           require_cn: bool, unknown_dir: str) -> str:
    """返回放弃理由（空串 = 可以通过，允许落盘）。

    require_cn 参数保留仅为兼容旧调用，已不再参与阻断判定（见上方说明）。
    """
    _ = require_cn
